...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    def loop(tweets: TweetSet, acc: TweetSet): TweetSet = {
      if(tweets.isEmpty) acc
      else if(!acc.contains(tweets.head)) loop(tweets.tail, acc.incl(tweets.head))
      else loop(tweets.tail, acc)
    }
    loop(that, this)
  }
  ...
  def ascendingByRetweet: Trending = {
    def loop(tweets: TweetSet, currentTrend: Trending): Trending = {
      if(tweets.isEmpty) currentTrend
      else loop(tweets.tail, currentTrend + tweets.findMin)
    }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    def loop(tweets: TweetSet, acc: TweetSet): TweetSet = {
      if(tweets.isEmpty) acc
      else if(p(tweets.head)) loop(tweets.tail, acc.incl(tweets.head))
      else loop(tweets.tail, acc)
    }
    loop(left.union(right).incl(elem), accu)
  }
...
object GoogleVsApple {
  ...
  private def search(text: String, keywords: List[String]): Boolean = {
    if(keywords.isEmpty) false
    else if(keywords.contains(text) || text.contains(keywords.head)) true
    else search(text, keywords.tail)
  }

  val googleTweets: TweetSet = {
    TweetReader.allTweets.filter { tweet =>
      search(tweet.text, google)
    }
  }

  val appleTweets: TweetSet = {
    TweetReader.allTweets.filter { tweet =>
      search(tweet.text, apple)
    }
  }
